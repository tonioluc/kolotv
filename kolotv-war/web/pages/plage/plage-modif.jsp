
<%@ page import="affichage.*"%>
<%@ page import="plage.Plage" %>
<%@ page import="categorieheure.CategorieHeure" %>

<%
    Plage t = new Plage();
    String  mapping = "plage.Plage",
            nomtable = "PLAGE",
            apres = "plage/plage-fiche.jsp",
            titre = "Modification Plage";

    PageUpdate pu = new PageUpdate(t, request, (user.UserEJB) session.getValue("u"));
    pu.setLien((String) session.getValue("lien"));
    pu.setTitre(titre);

    pu.getFormu().getChamp("heureDebut").setLibelle("Heure d&eacute;but");
    pu.getFormu().getChamp("heureDebut").setType("time");
    pu.getFormu().getChamp("heureDebut").setAutre("step=\"1\"");
    pu.getFormu().getChamp("heureFin").setLibelle("Heure de fin");
    pu.getFormu().getChamp("heureFin").setType("time");
    pu.getFormu().getChamp("heureFin").setAutre("step=\"1\"");
    pu.getFormu().getChamp("daty").setLibelle("Date");
    pu.getFormu().getChamp("idSupport").setLibelle("Support");
    pu.getFormu().getChamp("idCategorieHeure").setLibelle("Cat&eacute;gorie de l'heure");

    Liste[] liste = new Liste[2];
    CategorieHeure mp = new CategorieHeure();
    liste[0] = new Liste("idCategorieHeure", mp,"desce","id");
    liste[0].setLibelle("Cat&eacute;gorie de l'heure");
    liste[1]=new Liste("jour");
    String[] joursString = new String[] {"Lundi","Mardi","Mercredi","Jeudi","Vendredi","Samedi","Dimanche"};
    liste[1].makeListeString( joursString,  joursString);
    pu.getFormu().changerEnChamp(liste);

    pu.getFormu().getChamp("idSupport").setPageAppelComplete("support.Support","id","SUPPORT");

    String lien = (String) session.getValue("lien");
    String id=pu.getBase().getTuppleID();
    pu.preparerDataFormu();
%>
<div class="content-wrapper">
    <div class="row">
        <div class="col-md-3"></div>
        <div class="col-md-6">
            <div class="box-fiche">
                <div class="box">
                    <div class="box-title with-border">
                        <h1 class="box-title"><a href=<%= lien + "?but=plage/plage-fiche.jsp&id="+id%> <i class="fa fa-arrow-circle-left"></i></a><%=pu.getTitre()%></h1>
                    </div>
                    <form action="<%= lien %>?but=apresTarif.jsp&id=<%out.print(request.getParameter("id"));%>" method="post">
                        <%
                            out.println(pu.getFormu().getHtmlInsert());
                        %>
                        <div class="row">
                            <div class="col-md-11">
                                <button class="btn btn-primary pull-right" name="Submit2" type="submit">Valider</button>
                            </div>
                            <br><br>
                        </div>
                        <input name="acte" type="hidden" id="acte" value="update">
                        <input name="bute" type="hidden" id="bute" value="<%=apres%>">
                        <input name="classe" type="hidden" id="classe" value="<%=mapping%>">
                        <input name="rajoutLien" type="hidden" id="rajoutLien" value="id-<%out.print(request.getParameter("id"));%>" >
                        <input name="nomtable" type="hidden" id="nomtable" value="<%=nomtable%>">
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>
